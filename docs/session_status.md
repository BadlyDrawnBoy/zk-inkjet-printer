...
* **Helper scripts added under `ghidra_scripts/`**
  * `export_io_callgraph.py` – emits a depth-limited call graph from a given
    entry point (current root: `0X20EAEC`, the upgrade orchestrator). Output
...
* `ghidra_scripts/export_io_callgraph.py` invoked successfully but the
  ...
  (`0X20EAEC`). No outgoing edges were recorded.
...
1. **Fix the disassembly mode around `0X20EAEC`**
   * Set the `TMode` context bit to `1` (Thumb) for this region and re-run
     ...
     value at `0X20EAEC`. Headless-only option: write a script that applies a
...
4. **MMIO write inventory**
   * Once real code flow is visible, search the subgraph for translated
     `STR/STM` operations that access bases ≥ `0XB0000000`. Document these in
...
### Current Findings
- The address range `0X20EAEC`–`0X20EB0C` is all zero after the `v5t` re-import (`dump_bytes.py 0X20EAEC 32`). This indicates the bytes were not mapped from the raw image when the new program was created.
...
1. **Verify mapping for the orchestrator block**  
   ...
   - If the address space truncates before `0X20EAEC`, adjust the raw binary loader options (start offset / length) or use “Memory Map” to add the missing bytes manually.
...
- Disassembled the ARM callee at `0X211B7C` by extracting 256 bytes and running:
...
### Current Findings
- Despite fresh imports, explicit `set_image_base.py`, and chunk/FileBytes remapping scripts, every headless `getBytes` in Ghidra still reports zeros for `0X20EAEC`. The new diagnostics confirm only a single `ram` block exists and is marked initialised/readable, so the failure is likely a program-database quirk rather than missing file data.
...
### Actions Performed
- Added `scripts/upgrade_orchestrator_callgraph.py`, a Capstone-based helper that decodes the Thumb routine at `0X20EAEC` straight from the raw APP image and emits both JSON (`data/processed/upgrade_orchestrator_calls.json`) and a text listing (`data/processed/upgrade_orchestrator_disasm.txt`).
...
- Spot-checked the direct callee at `0X211B7C` with Capstone in ARM mode to confirm it terminates with `bx lr` and contains the expected bitfield math used by prior notes.

### Current Findings
- The JSON artifact captures the orchestrator loop that seeds a 0X20-entry table on the stack before invoking the staging helpers. Register calls at `0X20EB52` (`blx r4`) and `0X20EB68` (`blx r7`) pull their targets from `[sp, #0X8c]`, proving the runtime supplies a function pointer table rather than fixed ROM addresses.
...
- The ARM helper at `0X211B7C` is confirmed live and should be catalogued in the descriptor/MMIO notes once the pointer table decoding is complete.
...
### Actions Performed
- Authored `scripts/upgrade_orchestrator_callers.py` to scan the raw APP binary with Capstone and enumerate `bl`/`blx` instructions that land on the Thumb orchestrator at `0X20EAEC`.
- Ran the scanner (default target) to confirm only one direct call site exists: `0X20C75C` (`bl #0X20eaec`) inside the Thumb wrapper at `0X20C74C`.
...
- Disassembled the ARM routine referenced by the literal (`0XA1874`) and captured its behaviour: it populates offsets `0X40–0X43` and `0X224–0X22f` with ASCII markers and splits fields from `[r4+0X18]`/`[r4+0X1c]`, hinting at a UI/status preparation step.

### Current Findings
- The orchestrator wrapper (`0X20C74C`) is invoked from `0X243DD0`, which seeds `r0` with the literal at `0X243DF0` (`0XA1874`) and passes `sp` in `r1`. Consequently, the orchestrator receives:
  * `r0` – dereferenced pointer returned by the global stub at `0X20EA38` (still sourced from `VA 0X244F8C (file+0X44F8C)`).
  * `r1` – function pointer `0XA1874` (ARM mode), reused for both `blx r4` and `blx r7`.
  * `r2` – scratch pointer into the caller’s stack frame.
- ARM routine `0XA1874` appears to stage UI text/metadata, writing `"RR"`/`"aA"` pairs and unpacking two 32-bit fields into byte slices before invoking helper calls at `0X48610` and `0X48504`.
...
### Suggested Next Moves
1. Decode the structure at `VA 0X244F8C (file+0X44F8C)` (the pointer returned by `0X20EA38`) so we know which table the orchestrator walks before issuing the scripted callbacks.
2. Classify the ARM helper at `0XA1874` and its callees (`0X48610`, `0X48504`) in `docs/app_message_handlers.md`, noting how they transform the UI/state buffer passed via `r1=sp`.
...
### Actions Performed
- Reverse-engineered the data structure referenced via `0X20EA38`/`VA 0X244F8C (file+0X44F8C)`. The stub returns a pointer to a singly linked list of *free-block descriptors* that lives in RAM (first word = block size in bytes, `+0X4` = next node, `+0XC` feeds the tail pointer used later). The orchestrator iterates this list, bins block lengths by bit-width, and tallies total free bytes before feeding the results to its callbacks.
...
- Analysed the callback target at file offset `0XA17C8` (load address `0X20A17C8`). It treats `r0` as a 0X240-byte UI/status buffer, writes the `"RR"` / `"aA"` banner prefix, serialises the 32-bit counters from `[base+0X18]`/`[base+0X1C]` into ASCII bytes at `[base+0X228..0X22F]`, increments `[base+0X3C]` (block counter), and then invokes the hardware helpers at `0X48610` / `0X48504` to push the formatted line to the screen. Failure in the T5L helper keeps the status flag (`[base+3]`) asserted so the UI can retry.
...
### Current Findings
- The global at `VA 0X244F8C (file+0X44F8C)` is a pointer slot that the runtime populates with the head of the free-block list (`struct list_node { uint32_t length; struct list_node *next; uint32_t payload; uint32_t tail; }`). The orchestrator stores `head + 0XC` on the stack but does not dereference it yet; companion routines likely use the tail pointer to append new nodes.
- Callback #1 (`0XA17C8`) clears and rebuilds the status blink buffer, ensuring the `"RR aA"` prefix and the packed byte counters reflect the latest histogram totals before issuing the display DMA transfers.
...
- No additional callers were found beyond the wrapper at `0X20C74C`, confirming the orchestrator list is exclusively consumed by the upgrade status path.

### Suggested Next Moves
1. Observe the runtime initialisation that seeds `VA 0X244F8C (file+0X44F8C)` to confirm when the free-block list is allocated and whether `+0XC` truly holds the tail pointer (instrumenting the headless project or emulating the linked list builder would help).
2. Capture the data pumped into `0X48610`/`0X48504` to determine the exact hardware protocol (likely T5L DMA descriptors) and cross-link the offsets with `docs/mmio_writes.md`.
...
### Actions Performed
- Searched for `bl`/`blx` calls into the queue dispatcher (`0X211366`). Found only two direct callers: the tiny wrapper at `0X21135E` and a large Thumb routine at `0X229D78`.
- Disassembled the wrapper (`0X21135E`), confirming it is just `queue_init_and_dispatch()`—it invokes the lazy initialiser (`0X21138A`) and then falls straight through to the dispatcher. No additional state is mutated here.
- Examined the 0X229D78 caller. The function sets up work descriptors on the stack, generates output chunks via callback pointers held in `[sp+0X10]`/`[sp+0X16]`, and when a “flush” flag at `[sp]` is non-zero it:
  ...
  * Immediately invokes `0X211AD8`, which appears to reset the optional tail buffer acquired earlier (it issues `0X20C804`, the same Thumb helper used when recycling nodes).
- Mapped the auxiliary helpers in the same region:
  * `0X229C80`/`0X229C9A` bump and drop the 0X1C-byte reference counter stored in each node; when that counter hits zero they call `0X2278D0` followed by `0X229D3C` to return the block to the pool.
  * `0X229CD8` offsets the descriptor by 0X10 and forwards to `0X22987C`, showing that the first 16 bytes of the queue node form a header preceding the data payload.

### Current Findings
- The queue dispatch sequence is only triggered from the large work routine at `0X229D78`, which packages variable-length data and writes stride/callback metadata into the tail of the node before flushing it through `queue_dispatch()`. This routine also populates the auxiliary fields at offsets `0X78/0X7C` and iterates `*(node+0X16)` to stream multi-block updates.
- Reference counting for queue nodes is tracked at offset `0X1C`; the helper pair at `0X229C80`/`0X229C9A` increments the new owner’s count and decrements the previous owner, returning the block to the allocator via `0X229D3C` when the count reaches zero.
- Still no direct calls to the heavy processor at `0X211430`; the address is almost certainly stored in the vtable slot seeded by the literal `0X2113CC`, so the function is reached via function-pointer dispatch rather than a static `bl`.
...
| `[sp+0X2C]` | Prefill callback | Supplies initial write pointer (`0X211356` path). |
...
Current slot contents (mirrored in `docs/offset_catalog.md`): `queue_ctrl+0X8` carries the default prefill callback at `0X269385` (Thumb) that `queue_prefill` falls back to, while the one-shot hook at `queue_ctrl+0X10` is still cleared during initialisation – the concrete writer remains TBD.

`queue_prefill` (`0X211356`) fetches its callbacks directly from the controller block: it first probes `queue_ctrl+0X10` (optional prefill handler) and, when that slot is null, falls back to the default pointer stored at `queue_ctrl+0X8`.

*Optional RAM hook plan (not yet implemented):* patch tail of `0X21135E` (`queue_init_and_dispatch`) to check for `3:/DUMP.MRK`; when present, emit `0X244F00–0X245000` as `ram_qctrl.bin` and `0X217E800–0X217EC00` as `ram_lit.bin`. The helper would reuse the queue’s installed read callback (reachable via `0X211356`) to dump the controller state, then flush the artefacts through the existing file wrappers (`0X216B78` open/write, `0X215C64` close/reset). Remember the controller block lives at `VA 0X244F8C (file+0X44F8C)`.

### Suggested Next Moves
1. Resolve the vtable literal at `0X2113CC` and locate who installs actual handlers into `[queue_ctrl+0X10]`/`[queue_ctrl+0X18]` so we can trace where `0X211430` is invoked.
2. Analyse `0X229D78`’s parameters (stack layout around `[sp+0X0]…[sp+0X48]`) to document the API that produces queue entries—this will make it easier to recreate or patch the free-block data path.
3. Investigate `0X211AD8` to confirm whether it clears the scratch buffer or performs additional bookkeeping after each dispatch; note any MMIO writes it emits.
4. Optional RAM dump scaffold: add a `BL maybe_dump` at the tail of `queue_init_and_dispatch` (`0X21135E`) guarded by a marker file (`3:/DUMP.MRK`). The helper would call the existing file I/O wrappers around `0X216B78`/`0X215C64` to emit two slices – `0X244F00..0X245000` (`ram_qctrl.bin`) and `0X217E800..0X217EC00` (`ram_lit.bin`) – then disable itself via a static `dump_done` flag.
...
### Actions Performed
- Identified the queue-control accessor at `0X2113E4` (ARM stub). It returns a pointer to the controller block located at `VA 0X244F8C (file+0X44F8C)`. The first word of that block acts as the free-list head pointer consumed by the orchestrator.
- Disassembled the Thumb helper at `0X21138A`. On first use it allocates a 0X20-byte node via `0X20C798`, writes the new node back into `[queue_ctrl+0X0]`, zeroes the bookkeeping fields, and populates a vtable slot with the relocation literal stored alongside the function. The optional scratch buffer lives at `[node+0X1C]` and is only allocated when the literal at `0X2113D4` is non-zero (currently it is zero, so the tail pointer is initialised to `NULL`).
- Examined the descriptor allocator `0X211AFC` (Thumb). It reserves 0X88 bytes, clears the first byte, and returns the pointer; this is the helper invoked when the queue requires a scratch arena.
- Followed the immediate caller `0X211366`, confirming it defers to `0X21138A` for lazy initialisation before invoking the callback table stored at offsets `0X10/0X18/0X1C`.

### Current Findings
- The orchestrator’s “free-block queue” is backed by a small controller structure whose head pointer sits at `queue_ctrl[0]`. The initial node includes a vtable pointer (literal at `0X2113CC` resolves to Thumb target `0X208592`) and a constant `0X6` bucket count at offset `0X8`, matching the 32 histogram bins used by `0X20EAEC`.
- Node initialisation clears the status byte at offset `0XC` and the three pointer slots at offsets `0X8/0XC/0X10`. A secondary buffer (offset `0X1C`) is provisioned via `0X211AFC` when the optional literal is set—currently the firmware ships with this literal cleared, so the list starts empty beyond the head node.
- The accessor `0X211366` repeatedly calls function pointers stored at `queue_ctrl+0X10` and `queue_ctrl+0X18`, which aligns with the orchestrator’s later `blx r4` / `blx r7` dispatch; those registers are loaded from the stack slots the accessor fills.

### Suggested Next Moves
1. Track the callers of `0X211366` / `0X211430` to see where new blocks are appended to the free list, and document how `[node+0X4]` and `[node+0X1C]` evolve at runtime.
2. Resolve the relocation literal at `0X2113CC` once we have relocation data (or a RAM dump) to confirm which routine is being registered as the node vtable.
3. Map the remaining fields inside `queue_ctrl` (offsets `0X4`, `0X8`, `0X10`, etc.) by instrumenting their readers in `0X211430` so the histogram output in the orchestrator can be tied back to concrete queue state.
4. Optional: plan a “display tap” around `0X48610` / `0X48504` – either a NOP stub (mute DMA) or a RAM copy of the `(ptr,len)` arguments before tail-calling the original helper – so we can inspect UI payloads without JTAG once function ownership is confirmed.
