...
## Session 2025-01-25: SoC Identification
**Goal:** Identify the SoC used in ZK-INKJET printer  
**Result:** ✅ Nuvoton N3290X confirmed (95% confidence)  
**Details:** See `docs/sessions/session-2025-01-25-soc-identification.md`

**Key Commands:**
```bash
# USB String Descriptor found at 0x00475180
ghidra-bridge---read_bytes --address 0x00475140 --length 256

# Datasheet analysis
pdftotext N3290X.PDF - | grep -i "ARM926"
```
...
## Session 2025-11-03: GPIO Pins Analysis (GPB[2..5])
**Goal:** Identify firmware functions accessing GPIO Port B pins 2-5 (physical pins 125-128)  
**Result:** ✅ Pin-Mux and GPIO driver functions located, no I2S usage  
**Details:** See `docs/gpio_pins_analysis.md`

**Key Commands:**
```bash
# Search for register base addresses
ghidra-bridge---search_scalars --value 0xB0000000 --limit 50  # GCR_BA (54 hits)
ghidra-bridge---search_scalars --value 0xB8000000 --limit 50  # GPIO_BA (10 hits)
ghidra-bridge---search_scalars --value 0xB1001000 --limit 20  # I2S_BA (0 hits)

# Disassemble key GPIO functions
ghidra-bridge---disassemble_at --address 0x00031f34 --count 60  # Pin Mux Config
ghidra-bridge---disassemble_at --address 0x00031ec4 --count 25  # GPIO Read Pin
ghidra-bridge---disassemble_at --address 0x0003224c --count 30  # GPIO Write Pin

# List GPIO driver module functions
...
**Artifacts:**
- `docs/gpio_pins_analysis.md` - Complete analysis with register maps and function details
- Pin Mux Function: `FUN_00031f34` @ 0x00031f34 (600 bytes)
- GPIO Read Function: `FUN_00031ec4` @ 0x00031ec4 (96 bytes)
- GPIO Write Function: `FUN_0003224c` @ 0x0003224c (100 bytes)

**Key Findings:**
- GPB[2..5] used as standard GPIOs (not I2S)
- Unified GPIO driver with port-based dispatch
- Indirect addressing: base + offset (explains why full address searches failed)
- 54 GCR_BA uses, 10 GPIO_BA uses, 0 I2S_BA uses

---

## Session 2025-11-03 (Update): Alternative Functions Analysis for GPB[2-5]
**Goal:** Investigate if GPB[2-5] use alternative functions (UART, SPI, I2C, PWM, I2S)  
**Result:** ❌ GPB[2-5] are NOT configured - neither as GPIO nor alternative functions  
**Details:** Systematic analysis of all pin-mux function calls

**Key Commands:**
```bash
# Analyze pin-mux function
ghidra-bridge---disassemble_at --address 0x00031f34 --count 80

# Find all callers of pin-mux function
ghidra-bridge---search_xrefs_to --address 0x00031f34 --query "*" --limit 100

# Analyze each caller for Port B configurations
ghidra-bridge---disassemble_at --address 0x0005c1a0 --count 64  # Found GPB[6]
ghidra-bridge---disassemble_at --address 0x00031e0c --count 64  # Found GPB[0]

# Search for alternative function base addresses
ghidra-bridge---search_scalars --value 0xB8008000 --limit 50  # UART (0 hits)
ghidra-bridge---search_strings --query "SPI" --limit 50       # Found SPI flash refs
ghidra-bridge---search_strings --query "UART" --limit 50      # No UART strings
ghidra-bridge---search_strings --query "PWM" --limit 50       # No PWM strings
```

**Artifacts:**
- Updated `docs/gpio_pins_analysis.md` with corrected findings
- Section 8: "Unconfigured Pins Analysis (GPB[2-5])"

**Key Findings:**
- Only 3 Port B configurations found in 12 total pin-mux calls:
  - GPB[0]: Function 2 (2 locations)
  - GPB[6]: Function 5 (1 location)
- GPB[2-5]: No configuration found
- No alternative functions (UART/SPI/I2C/PWM/I2S) for GPB[2-5]
- Likely explanation: Pins remain in default state or unused in hardware

**Correction to Previous Analysis:**
- Previous conclusion "GPB[2-5] used as standard GPIOs" was incorrect
- Corrected to: "GPB[2-5] are NOT configured in firmware"
...
## Session 2025-11-03 (Audit): Repository Verification Status
**Goal:** Audit all documentation, distinguish facts from hypotheses, update chip identification  
**Result:** ✅ Complete audit with confidence levels, corrected chip ID to N32903K5DN  
...
- GPB[2-5] pin usage
- Chip identification (N3290X → N32903K5DN)

**Confidence Improvement:**
- SoC identification: 95% → 98%
- GPIO analysis: 95% → 98%

---

## Session 2025-11-03 (Correction): MMIO Address Verification
**Goal:** Verify MMIO addresses from previous analysis, correct false positives  
**Result:** ✅ Corrected 0xB1006800 → 0xB800C000, verified Doorbell/Parameter Block  
**Details:** Systematic verification of MMIO fingerprint

**Key Commands:**
```bash
# Search for 0xB1006800 (claimed Doorbell address)
ghidra-bridge---search_scalars --value 0xB1006800 --limit 50  # 0 hits - does not exist!

# Search for 0x204 offset (Clock/Bus control)
ghidra-bridge---search_scalars --value 0x204 --limit 50  # 127 hits

# Verify GCR_BA + 0x204
ghidra-bridge---disassemble_at --address 0x0006fc20 --count 16
# Result: mov r4,#0xb0000000; ldr r0,[r4,#0x204] ✅

# Check literal pool for Doorbell base
ghidra-bridge---read_bytes --address 0x00270188 --length 4
# Result: 0xB800C000 (not 0xB1006800!)

# Analyze Doorbell usage pattern
ghidra-bridge---disassemble_at --address 0x00270040 --count 64
# Result: Poll bit 0, write params @ +0x10/+0x14/+0x18/+0x1C, write cmd @ +0x0 ✅

# Verify Display Controller
ghidra-bridge---read_bytes --address 0x00230f34 --length 4
# Result: 0xB100D000 ✅
```

**Findings:**
- ❌ **0xB1006800 does NOT exist** in firmware (0 references)
- ✅ **0xB800C000 is the Doorbell/Parameter Block** (verified @ 0x00270040)
- ✅ **0xB0000204 is Clock/Bus Control** (verified @ 0x0006fc24)
- ✅ **0xB100D000 is Display Controller** (verified @ 0x00230E04)

**Pattern Analysis - Doorbell @ 0xB800C000:**
```
1. Poll: ldr r10,[r12,#0x0]; tst r10,#0x1; bne (wait loop)
2. Write params: str r9,[r12,#0x10]; str r8,[r12,#0x14]; ...
3. Write command: str r5,[r12,#0x0]
```
...
- 0xB1006800 (Doorbell) → 0xB800C000 (Doorbell) ✅
- 0xB800C000 (FIFO/Status) → 0xB800C000 (Doorbell) ✅

**Confidence Improvement:**
- Doorbell/Parameter Block: 75% → 95%
- MMIO Fingerprint accuracy: Significantly improved
